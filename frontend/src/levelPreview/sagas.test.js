...
import { makeMoveSaga, undoMoveSaga, setInitialStateSaga } from './sagas';
...
describe('The undo move saga', () => {
  it('Updates the game states until the entities have stopped moving', () => {
...
describe('The set initial state saga', () => {
  it('Sets the initial game state', () => {
    const generator = setInitialStateSaga();
    ...
    //  Get the current game speed from the store
    let step = generator.next();
    expect(step.done).toBe(false);
    expect(step.value).toEqual(select());

    //  The next action should be a pause
    step = generator.next(state);
    expect(step.done).toBe(false);
    expect(step.value).toEqual(call(delay, DEFAULT_GAME_SPEED));

    //  The next action should be to make a move
    step = generator.next();
    expect(step.done).toBe(false);
    //  expect(step.value).toEqual(put(undoMoveFinished(MOVE_NONE))); //  @FIXME
